...
3.  **複雜案件審核與分發流程 (核心業務邏輯)**
    ...
    * **流程圖佐證：** 案件從申請到分發的複雜狀態機請參考 [案件審核 流程圖 .png]。

---

## ⚙️ 環境啟動與部署指南 (Getting Started)
...
### 這份 docker-compose.yml 檔案清晰地展示了您的容器化策略，確認了只有 React 前端和 MailHog 處於 Docker 環境中。

我將把這份 YAML 文件的專業細節整合到 README.md 的「環境啟動與部署指南」中，特別強調您對前端開發環境的優化。

🚀 最終優化後的 Markdown (包含 YAML 細節)
請將以下內容替換您整個 README.md，這份是您作品集最完整、最有說服力的版本。
...
| **資料持久層** | **Spring Data JPA/Hibernate** | 簡化資料庫操作，專注於物件關係映射 (ORM)。 |
| **前端核心** | **React** (JavaScript) | 負責用戶介面 (UI) 和使用者互動體驗。 |
| **環境與容器** | **Docker / Docker Compose** | 用於前端開發環境與輔助服務容器化。 |
| **輔助工具** | **MailHog** | 提供本地郵件伺服器，用於開發時的郵件通知測試。 |
...
* **標準化三層架構：** 嚴格區分 **Controller**、**Service**、**Repository** 的職責。
* **資料物件轉換：** 專門使用 **`converter`** 套件，並透過 **`ModelMapperConfig`** 配置，集中處理 DTO 與 Model 之間的資料映射。
* **統一響應與異常處理：**
    * 實作 **`GlobalExceptionHandler`**，並使用 **`response`** 套件中的 **`ApiResponse`** 統一格式化所有 API 的成功或錯誤響應。
