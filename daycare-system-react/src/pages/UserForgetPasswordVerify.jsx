...
import "./UserForgetPasswordVerify.css"

// 假設 verifyAccount 來自您的服務層
import { forgetPasswordVerifty } from '../services/emailService';

function ForgetPasswordVerifty() {
  ...
  useEffect(() => {
    ...
    const handleVerification = async () => {
      ...
      try {
        // 執行 API 呼叫 (傳遞 token)
        const response = await forgetPasswordVerifty(token);

        const username = response.data.username;

        // --- 成功邏輯 ---
        const message = '認證成功';
        setStatus(message + ' 3 秒後將自動跳轉至修改密碼頁面...');
        setIsSuccess(true);

        // 成功後延遲 3 秒跳轉
        setTimeout(() => {
          navigate(`/forget/password?username=${username}&token=${token}`);
        }, 3000);
...
export default ForgetPasswordVerifty;