 ...
 */
@Getter
public enum CaseOrganizationStatus implements BaseEnum{

	// --- 進行中 / 已申請 ---

	// 申請中 (Pending Review)
	APPLIED("申請中"), // 剛提交，等待審核/抽籤
	...
	// 未通過 (Rejected)
	REJECTED("未通過"), // 經審核/抽籤後，本機構確定不錄取

	// --- 暫時狀態 / 被動狀態 ---

	// 備選中 (Waitlisted)
	WAITLISTED("備取中"), // 經抽籤後，本機構列為備取名單

	// 其他機構錄取 (Passed by Other Organization)
	// 這是針對「志願序」而言，如果第一志願通過，第二志願會轉為這個狀態。
	ALLOCATED_TO_OTHER("其他機構錄取"), 

	// 撤回申請 (Withdrawn by Public User)
	WITHDRAWN("撤回"); // 民眾主動撤回申請

	private final String description; // 狀態的中文描述

	CaseOrganizationStatus(String description) {
	...
	public static CaseOrganizationStatus fromCode(String code) {
		return BaseEnum.formCode(CaseOrganizationStatus.class, code);
	}
}
